...
export default function ForgotPasswordPage() {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resetError) {
        setError(resetError.message || "Gagal mengirimkan tautan atur ulang sandi.");
      } else {
        setIsSuccess(true);
      }
    } catch (err) {
      ...
      >
        ...
        <Card className="backdrop-blur-xl bg-white/60 dark:bg-zinc-900/60 border-zinc-200/50 dark:border-zinc-800/50 shadow-2xl">
          <CardHeader className="space-y-1 text-center">
            ...
            <CardDescription>
              Masukkan alamat email Anda untuk menerima tautan atur ulang password.
            </CardDescription>
          </CardHeader>
            ...
            <CardContent className="space-y-6 pt-4 pb-8">
              <div className="flex flex-col items-center justify-center space-y-4 text-center">
                ...
                <div className="space-y-2">
                  <h3 className="text-lg font-semibold">Tautan Terkirim!</h3>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">
                    Silakan periksa email <span className="font-medium text-zinc-900 dark:text-zinc-100">{email}</span> untuk mengatur ulang sandi Anda.
                  </p>
                  <p className="text-xs text-orange-500 font-medium mt-2">
                    (Catatan Mode Dev: Cek terminal/console untuk tautan reset)
                  </p>
                </div>
              </div>
