...
import { Activity, Loader2, Wand2, CheckCircle2, ArrowLeft } from "lucide-react";
...
export default function MagicLinkPage() {
  ...
  const handleSendMagicLink = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: magicLinkError } = await signIn.magicLink({
        email,
        callbackURL: "/dashboard"
      });

      if (magicLinkError) {
        setError(magicLinkError.message || "Gagal mengirimkan magic link. Pastikan email Anda terdaftar.");
      } else {
        setIsSuccess(true);
      }
    } catch (err) {
      ...
      >
        ...
        <Card className="backdrop-blur-xl bg-white/60 dark:bg-zinc-900/60 border-zinc-200/50 dark:border-zinc-800/50 shadow-2xl">
          <CardHeader className="space-y-1 text-center">
            ...
            <CardDescription>
              Masuk tanpa sandi. Kami akan mengirimkan tautan ajaib ke email Anda.
            </CardDescription>
          </CardHeader>
            ...
            <CardContent className="space-y-6 pt-4 pb-8">
              <div className="flex flex-col items-center justify-center space-y-4 text-center">
                ...
                <div className="space-y-2">
                  <h3 className="text-lg font-semibold">Tautan Ajaib Terkirim!</h3>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">
                    Silakan periksa email <span className="font-medium text-zinc-900 dark:text-zinc-100">{email}</span> dan klik tautan untuk masuk secara otomatis.
                  </p>
                  <p className="text-xs text-orange-500 font-medium mt-2">
                    (Catatan Mode Dev: Cek terminal/console untuk tautan login)
                  </p>
                </div>
              </div>
              <Link href="/login" className={buttonVariants({ variant: "outline", className: "w-full mt-4" })}>
            ...
            <form onSubmit={handleSendMagicLink}>
              ...
              <CardFooter className="flex flex-col space-y-4">
                ...
                <div className="text-center">
                  <Link href="/login" className="text-sm font-medium text-zinc-500 dark:text-zinc-400 hover:text-primary transition-colors flex items-center justify-center gap-1">
